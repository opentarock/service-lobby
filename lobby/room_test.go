...
func TestUserCanJoinRoom(t *testing.T) {
	...
	assert.Contains(t, room.GetUserIds(), user.Id("2"))
}
...
func TestOwnerCanLeaveRoom(t *testing.T) {
	...
	room.Leave(ownerId)
	assert.Equal(t, 1, room.NumPlayers())
	assert.NotContains(t, room.GetUserIds(), ownerId)
	assert.Equal(t, user.Id("2"), room.GetOwner(), "New owner is selected when the current owner leaves")
}
...
func TestReturnsUserIds(t *testing.T) {
	...
	room.Join("3")
	assert.Contains(t, room.GetUserIds(), user.Id("3"))
	assert.Contains(t, room.GetUserIds(), user.Id("2"))
	assert.Contains(t, room.GetUserIds(), ownerId)
...
func TestReturnsNonOwnerUserIds(t *testing.T) {
	...
	assert.Contains(t, room.GetNonOwnerUserIds(), user.Id("2"))
	assert.NotContains(t, room.GetNonOwnerUserIds(), ownerId)

	room.Leave("2")
	assert.NotContains(t, room.GetNonOwnerUserIds(), user.Id("2"))
	assert.NotContains(t, room.GetNonOwnerUserIds(), ownerId)
}
